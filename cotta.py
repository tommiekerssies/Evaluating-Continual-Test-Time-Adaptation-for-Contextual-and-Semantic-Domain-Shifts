...
class CoTTA(Module):
  ...
  def forward_and_adapt(self, x):
    ...
    # Teacher update
    self.model_ema = update_ema_variables(ema_model=self.model_ema, model=self.model, alpha_teacher=self.mt_alpha)
    # Stochastic restore
    for nm, m in self.model.named_modules():
      for npp, p in m.named_parameters():
        if npp in ['weight', 'bias'] and p.requires_grad:
          mask = (torch.rand(p.shape) < self.rst_m).float().to(self.device)
          with torch.no_grad():
  ...
  def __deepcopy__(self, memo):
    ...
    ema_state_dict = self.model_ema.state_dict()
    ...
    self.model_ema.load_state_dict(ema_state_dict)
    
    cp.model_ema = deepcopy(model)
    cp.model_ema.load_state_dict(ema_state_dict)
    for param in cp.model_ema.parameters():
      param.detach_()
